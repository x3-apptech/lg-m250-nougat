...
#define LGE_SUPERBLOCK_RECOVERY
...
#ifdef LGE_SUPERBLOCK_RECOVERY
        /*
        modify -b option util the filesystem has no error with superblock
        If e2fsck runs with -b option,
        1. it tries to recover target partition's FS superblock with backup superblock specified with -b option.
        2-1. if FS superblock has been recovered returning exit_value 'FSCK_OK' or 'FSCK_NONDESTRUCT'
             which mean the FS is clear and good to use,
             the repeat block below isn't going to be used since e2fsck's job is successfully done.
        2-2. if FS superblock hasn't been recovered even though e2fsk ran with backup superblock
             specified with -b option,
        it's time for the repeat block to work!
        -first, check the other conditions if
        1. e2fsck is running with -b option. ->if((ctx->use_superblock) && (ctx->flags & E2F_FLAG_SB_SPECIFIED))
        2. the exit_value indicates recovery is unsuccessful. ->if(!(exit_value & FSCK_OK) && !(exit_value & FSCK_NONDESTRUCT))
        3. the target partitions' FS size is big enough to have other backup superblocks.
        -if all the conditions mentioned above is met, e2fsck will run until superblock gets recovered.
        The max trial is 20 if target partitions's FS has enough superblock
        */
        if((ctx->use_superblock) && (ctx->flags & E2F_FLAG_SB_SPECIFIED) &&
           (!(exit_value & FSCK_OK) && !(exit_value & FSCK_NONDESTRUCT)) &&  fs->group_desc_count > 1) {
                printf(_("superblock hasn't been corrected with backup superblock: %u \n"),ctx->use_superblock);
                printf(_("exit value was : %u \n"), exit_value);
                printf(_("grp_count : %d \n"), grp_cnt);

            if( grp_cnt < fs->group_desc_count && grp_cnt <=20 ){
                        grp_cnt+=2;
                        printf(_("grp_count : %d ,fs->group_desc_count : %d \n"), grp_cnt, fs->group_desc_count);
                        ctx->use_superblock = ctx->superblock + 32768 *2;
                        ctx->superblock = ctx->use_superblock;
                        printf(_("ctx->superblock   : %u  \n"), ctx->superblock);
                        printf(_("ctx->use_superblock   : %u  \n"), ctx->use_superblock);
                        ctx->flags |= E2F_FLAG_SB_SPECIFIED;
                        ctx->options |= E2F_OPT_YES;

                        printf(_("Restarting e2fsck with another backup Superblock : %u \n"), ctx->use_superblock);
                        retval = e2fsck_reset_context(ctx);
                        if (retval) {
                            com_err(ctx->program_name, retval, _("while resetting context"));
                            fatal_error(ctx, 0);
                        }
                        ext2fs_close(fs);
                        goto restart;
