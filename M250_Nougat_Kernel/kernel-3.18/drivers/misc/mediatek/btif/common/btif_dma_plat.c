			       ...
			       ENUM_BTIF_REG_ID flag)
{
    ...
    BTIF_DBG_FUNC("DMA's clock is on\n");
    BTIF_DBG_FUNC("Rx DMA's base address: 0x%lx\n", base);

	if (REG_RX_DMA_ALL == flag) {
        BTIF_DBG_FUNC("RX_EN(:0x%x\n", enable);
        BTIF_DBG_FUNC("RX_STOP:0x%x\n", stop);
        BTIF_DBG_FUNC("RX_FLUSH:0x%x\n", flush);
        BTIF_DBG_FUNC("INT_FLAG:0x%x\n", int_flag);
        BTIF_DBG_FUNC("RX_WPT:0x%x\n", wpt);
        BTIF_DBG_FUNC("RX_RPT:0x%x\n", rpt);
        BTIF_DBG_FUNC("INT_BUF_SIZE:0x%x\n", int_buf);
        BTIF_DBG_FUNC("VALID_SIZE:0x%x\n", valid_size);
        BTIF_DBG_FUNC("INT_EN:0x%x\n",
			       BTIF_READ32(RX_DMA_INT_EN(base)));
        BTIF_DBG_FUNC("RX_RST:0x%x\n", BTIF_READ32(RX_DMA_RST(base)));
        BTIF_DBG_FUNC("VFF_ADDR:0x%x\n",
			       BTIF_READ32(RX_DMA_VFF_ADDR(base)));
        BTIF_DBG_FUNC("VFF_LEN:0x%x\n",
			       BTIF_READ32(RX_DMA_VFF_LEN(base)));
        BTIF_DBG_FUNC("RX_THRE:0x%x\n",
			       BTIF_READ32(RX_DMA_VFF_THRE(base)));
        BTIF_DBG_FUNC("RX_FLOW_CTRL_THRE:0x%x\n",
			       BTIF_READ32(RX_DMA_FLOW_CTRL_THRE(base)));
        BTIF_DBG_FUNC("LEFT_SIZE:0x%x\n",
			       BTIF_READ32(RX_DMA_VFF_LEFT_SIZE(base)));
        BTIF_DBG_FUNC("DBG_STATUS:0x%x\n",
