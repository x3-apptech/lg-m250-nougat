            ...
            unsigned char reg_addr, unsigned char *data)
{
        ...
        for ( i = 0; i < 5; i++)
        {
            ...
            {
                pr_warn("1","[lm3648] : lm3648_smbus_write_byte fail: %d\n",ret_val);
...
void lm3648_flash_strobe_prepare(char OnOff,char ActiveHigh)
{
        ...
        down_interruptible(&lm3648_lock); // Add semaphore for lcd and flash i2c communication.
...
void lm3648_flash_strobe_en(void)
{
        ...
        int flash_OnOff=0;
        down_interruptible(&lm3648_lock); // Add semaphore for lcd and flash i2c communication.
        err = lm3648_smbus_read_byte(new_client_lm3648, 0x01, &flash_OnOff);
        ...
        if(lm3648_flash_ctrl == 1)
	        ...
	        flash_OnOff &= 0x00;
        err = lm3648_smbus_write_byte(new_client_lm3648, 0x01, &flash_OnOff);
        up(&lm3648_lock); // Add semaphore for lcd and flash i2c communication.
}
