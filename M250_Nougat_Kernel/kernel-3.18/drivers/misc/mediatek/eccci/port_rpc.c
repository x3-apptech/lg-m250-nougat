...
#ifdef BOOTUP_LOG_MTK_ORIGINAL
	CCCI_BOOTUP_LOG(0, RPC, "looking for %s id, len %d\n", gpio_name, len);
#else
	CCCI_LGE_BOOT_LOG(0, RPC, "looking for %s id, len %d\n", gpio_name, len);
#endif
...
#else
	CCCI_LGE_BOOT_LOG(0, RPC, "%s id %d\n", gpio_name, gpio_id);
#endif
				     ...
				     struct ccci_rpc_gpio_adc_output_v2 *output)
{
	...
	unsigned int val, i;
        ...
        CCCI_LGE_BOOT_LOG(0, RPC, "enter %s function\n", __func__);
		...
		for (i = 0; i < GPIO_MAX_COUNT_V2; i++) {
			if (input->gpioValidPinMask & (1 << i)) {
				...
				if (num >= 0) {
					...
					val = get_md_gpio_val(num);
					output->gpioPinValue[i] = val;
					CCCI_LGE_BOOT_LOG(0, RPC, "********* gpio pin num : %d, pin value : %d\n",
                                                                output->gpioPinNum[i], output->gpioPinValue[i]);
				}
			}
		}
	} else {
		if (input->reqMask & RPC_REQ_GPIO_PIN) {
			for (i = 0; i < GPIO_MAX_COUNT_V2; i++) {
				if (input->gpioValidPinMask & (1 << i)) {
					...
					if (num >= 0) {
						output->gpioPinNum[i] = num;
						CCCI_LGE_BOOT_LOG(0, RPC, "********* output gpioPinNum : %d, num : %d\n",
                                                                output->gpioPinNum[i], num);
					}
				}
			}
		...
		if (input->reqMask & RPC_REQ_GPIO_VALUE) {
			for (i = 0; i < GPIO_MAX_COUNT_V2; i++) {
				if (input->gpioValidPinMask & (1 << i)) {
					val = get_md_gpio_val(input->gpioPinNum[i]);
					output->gpioPinValue[i] = val;
					CCCI_LGE_BOOT_LOG(0, RPC, "********* gpioPinValue : %d, value : %d\n",
                                                                output->gpioPinValue[i], val);
				}
			}
		}
	...
	CCCI_LGE_BOOT_LOG(0, RPC, "exit %s function\n", __func__);
}
...
#else
	CCCI_LGE_BOOT_LOG(md_id, RPC, "ccci_rpc_work_helper++ %d\n", p_rpc_buf->op_id);
#endif
	tmp_data[0] = 0;
...
#else
				CCCI_LGE_BOOT_LOG(md_id, RPC, "[0x%08X]: name:%s, len=%d, get_num:%d\n",
                                             p_rpc_buf->op_id, name, length, get_num);
#endif
				pkt_num = 0;
	...
	case IPC_RPC_GET_GPIO_ADC_OP:
		{
			...
			if (pkt_size == sizeof(struct ccci_rpc_gpio_adc_intput)) {
				...
				CCCI_BOOTUP_LOG(md_id, KERN, "IPC_RPC_GET_GPIO_ADC_OP request=%x\n",
					     input_v2->reqMask);
#else
				CCCI_LGE_BOOT_LOG(md_id, KERN, "IPC_RPC_GET_GPIO_ADC_OP request=%x\n",
                                             input_v2->reqMask);
#endif
