...
//#define HALLIC_IRQ_SUPPORT_ONLY_ONE_EDGE 	// falling or rising, not both falling + rising
...
#define STR_POUCH_STATE(s)			((s) == SMARTCOVER_POUCH_OPENED ? "open": "close")
...
int hallic_is_pouch_closed(void)
{
	...
	if (pouch_state == SMARTCOVER_POUCH_CLOSED) return 1;
	else return 0;
}
...
static void boot_cradle_det_func(void)
{
	...
	if (cradle->pdata->pouch_irq_gpio >= 0) {
		hallic_get_pouch_state(cradle->pdata->pouch_irq_gpio, &cradle->pouch_state);

		wake_lock_timeout(&cradle->wake_lock, msecs_to_jiffies(cradle->wake_lock_timeout_ms));
...
static void hallic_pouch_work_func(struct work_struct *work)
{
	...
	if (cradle->pouch_state != pouch_state) {
		...
		if (pouch_state == SMARTCOVER_POUCH_OPENED) {
			...
			irq_set_irq_type(cradle->pdata->pouch_irq, IRQF_TRIGGER_FALLING);
		}
		else {
			// if smartcover is closed, set gpio to detect open.
			irq_set_irq_type(cradle->pdata->pouch_irq, IRQF_TRIGGER_RISING);
		}
		spin_unlock_irqrestore(&cradle->lock, flags);
#endif
...
static void smart_cover_gpio_init(struct device *dev)
{
	...
	if (cradle->pdata->pouch_debounce > 0) {
		gpio_set_debounce(cradle->pdata->pouch_irq_gpio, cradle->pdata->pouch_debounce);
	}
	ret = request_irq(cradle->pdata->pouch_irq, hallic_pouch_irq_handler, irq_flag,
				"hallic_pouch-eint", NULL);
	if (ret) {
		pr_err("%s:error, fail to register irq handler, ret=%d", __func__, ret);
	}
...
static ssize_t cradle_pouch_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;

	if (cradle) {
		len = snprintf(buf, PAGE_SIZE, "cradle state : %d -> %s\n",
						cradle->pouch_state, STR_POUCH_STATE(cradle->pouch_state));
	}
...
static struct device_attribute cradle_pouch_attr   = __ATTR(pouch, S_IRUGO, cradle_pouch_state_show, NULL);
...
static void hallic_pen_work_func(struct work_struct *work)
{
	...
	if (cradle->pen_state != pen_state) {
		...
		if (pen_state == STYLUS_PEN_OUT) {
			...
			irq_set_irq_type(cradle->pdata->pen_irq, IRQF_TRIGGER_FALLING);
		}
		else {
			// if pen is in, set gpio to detect out
			irq_set_irq_type(cradle->pdata->pen_irq, IRQF_TRIGGER_RISING);
		}
		spin_unlock_irqrestore(&cradle->lock, flags);
#endif
...
static void stylus_pen_gpio_init(struct device *dev)
{
	...
	if (cradle->pdata->pen_debounce > 0) {
		gpio_set_debounce(cradle->pdata->pen_irq_gpio, cradle->pdata->pen_debounce);
	}
	ret = request_irq(cradle->pdata->pen_irq, hallic_pen_irq_handler, irq_flag,
				"hallic_pen-eint", NULL);
	if (ret) {
...
static struct device_attribute cradle_pen_attr   = __ATTR(pen, S_IRUGO, cradle_pen_state_show, NULL);
...
static void hallic_parse_dt(struct device *dev, struct hallic_platform_data *pdata)
{
	...
	if (node) {
		...
		if (debounce_cnt >= 2 && debounce_cnt <= ARRAY_SIZE(debounce)) {
			...
			pdata->pouch_irq = irq_of_parse_and_map(node, 0);	// first
			...
			if (debounce_cnt >= 4) {
				...
				pdata->pen_irq = irq_of_parse_and_map(node, 1);	// second

				pr_info("%s::pen_irq_gpio=%d, pen_irq=%d\n", __func__,
						pdata->pen_irq_gpio, pdata->pen_irq);
			}
#endif
		}
	} else {
		pr_err("%s::not found for %s\n", __func__, HALL_IC_COMPATIBLE);
	}
}
