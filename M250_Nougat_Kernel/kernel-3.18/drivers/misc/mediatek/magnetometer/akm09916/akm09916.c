...
{
	struct akm_i2c_data *dat = i2c_get_clientdata(this_client);
	char strbuf[SENSOR_DATA_SIZE];
	s16 data[3], raw_data[3];
	if((atomic_read(&open_flag) == 0) || (factory_mode == 1))
	{
		AKECS_SetMode_SngMeasure();
		msleep(10);
...
static ssize_t store_opmode(struct device_driver *ddri, const char *buf, size_t count)
{
	int opmode = 0;
	
	if(1 == sscanf(buf, "%d", &opmode))
	{
		AKECS_SetMode(opmode);
	}
	else
	{
		printk(KERN_ERR "invalid content: '%s', length = %zu\n", buf, count);
...
static DRIVER_ATTR(opmode,      S_IWUSR, NULL, store_opmode );
/*----------------------------------------------------------------------------*/
