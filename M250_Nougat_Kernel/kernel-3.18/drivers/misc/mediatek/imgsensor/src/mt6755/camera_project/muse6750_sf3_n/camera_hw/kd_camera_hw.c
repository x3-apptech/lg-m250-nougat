...
#define CUST_MAIN_DVDD_LDO MAIN_DVDD_LDO - AVDD
#endif
...
PowerCust PowerCustList = {
    {
        ...
        {GPIO_SUPPORTED,   GPIO_MODE_GPIO, Vol_Low},	/* MAIN_DVDD_LDO; */
...
PowerUp PowerOnList_0 = {
    {
        ...
        {SENSOR_DRVNAME_S5K4H9_MIPI_RAW,
            {
...
PowerUp PowerOnList_1 = {
    {
        {SENSOR_DRVNAME_IMX258_MIPI_RAW,
            {
                {SensorMCLK, Vol_Low, 0},
                {RST, Vol_Low, 0},
                {MAIN_DVDD_LDO, Vol_Low, 1},
                {AVDD, Vol_2800, 1},
                {DOVDD, Vol_1800, 1},
                {MAIN_DVDD_LDO, Vol_High, 1},
                {AFVDD, Vol_2800, 1},
                {SensorMCLK, Vol_High, 1},
                {RST, Vol_High, 2}
            },
...
BOOL hwpoweron(PowerInformation pwInfo, char *mode_name)
{
    ...
    } else if (pwInfo.PowerType == MAIN_DVDD_LDO) {
        if (pwInfo.Voltage == Vol_High) {
            if (mtkcam_gpio_set(pinSetIdx, pwInfo.PowerType, GPIO_OUT_ONE)) {
                PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
            }
        } else {
            if (mtkcam_gpio_set(pinSetIdx, pwInfo.PowerType, GPIO_OUT_ZERO)) {
                PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
            }
        }
...
BOOL hwpowerdown(PowerInformation pwInfo, char *mode_name)
{
    if (pwInfo.PowerType == AVDD) {
        ...
        //PK_DBG("hwPowerDown: MAIN_DVDD_LDO %d\n", pwInfo.Voltage);
        if (pwInfo.Voltage == Vol_High) {
            if (mtkcam_gpio_set(pinSetIdx, pwInfo.PowerType, GPIO_OUT_ONE)) {
                PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
            }
        ...
        char *mode_name)
{
    ...
    if(lge_get_board_revno() == HW_REV_0) {
        PowerOnList = PowerOnList_0;
        PowerOffList = PowerOffList_0;
    }
    else { // lge_get_board_revno() >= HW_REV_A
        PowerOnList = PowerOnList_1;
        PowerOffList = PowerOffList_1;
    }
    ...
    if (On) {
        ...
        for (pwListIdx = 0; pwListIdx < 16; pwListIdx++) {
            if (currSensorName && (PowerOffList.PowerSeq[pwListIdx].SensorName != NULL)
                    && (0 ==
                        strcmp(PowerOffList.PowerSeq[pwListIdx].SensorName,
                            currSensorName))) {
                ...
                for (pwIdx = 0; pwIdx < 10; pwIdx++) {
                    if (PowerOffList.PowerSeq[pwListIdx].PowerInfo[pwIdx].
                            PowerType != VDD_None) {
                        if (hwpowerdown
                                (PowerOffList.PowerSeq[pwListIdx].
                                 PowerInfo[pwIdx], mode_name) == FALSE)
                            goto _kdCISModulePowerOn_exit_;
                    } else {
                        /*PK_DBG("pwIdx=%d\n", pwIdx);*/
                ...
                break;
            } else if (PowerOffList.PowerSeq[pwListIdx].SensorName == NULL) {
                break;
            } else {
            }
        }
