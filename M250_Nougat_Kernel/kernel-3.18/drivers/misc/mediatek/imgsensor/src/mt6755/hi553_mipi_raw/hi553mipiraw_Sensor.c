...
void HI553_write_cmos_sensor_burst(hi553_short_t* para, kal_uint32 len, kal_uint32 slave_addr)
{
	...
	if(pPara == NULL)
{
		LOG_DEBUG("[HI553MIPI] ERROR!! pPara is Null!!\n");
		return;
	}
...
#endif
				LOG_DEBUG("i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);	
                return ERROR_NONE;
            }
			LOG_DEBUG("Read sensor id fail, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);	
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
...
static kal_uint32 close(void)
{
	...
	/*No Need to implement this function*/ 
        ...
        write_cmos_sensor(0x0a00, 0x0000); // LGE_CHANGE: [2016-06-17] kyunghun.oh@lge.com, stream off before power off(Recommend form hynix)
        ...
        case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
            {
                ...
                *pHorFOV = (0x56)<<16 | (0x56); // 16:9 = 67, 4:3 = 67 // (67)<<16 | (67)  //86 degree
