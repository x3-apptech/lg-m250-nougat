...
struct pinctrl_state *cam_mipiswt_en_h = NULL;
struct pinctrl_state *cam_mipiswt_en_l = NULL;
struct pinctrl_state *cam_mipiswt_sel_h = NULL;
struct pinctrl_state *cam_mipiswt_sel_l = NULL;
...
int mtkcam_gpio_init(struct platform_device *pdev)
{
	...
	cam_mipiswt_en_h = pinctrl_lookup_state(camctrl, "mipiswt_en_1");
	if (IS_ERR(cam_mipiswt_en_h)) {
		ret = PTR_ERR(cam_mipiswt_en_h);
		pr_debug("%s : pinctrl err, cam_mipiswt_en_h\n", __func__);
	}


	cam_mipiswt_en_l = pinctrl_lookup_state(camctrl, "mipiswt_en_0");
	if (IS_ERR(cam_mipiswt_en_l)) {
		ret = PTR_ERR(cam_mipiswt_en_l);
		pr_debug("%s : pinctrl err, cam_mipiswt_en_l\n", __func__);
	}

	cam_mipiswt_sel_h = pinctrl_lookup_state(camctrl, "mipiswt_sel_1");
	if (IS_ERR(cam_mipiswt_sel_h)) {
		ret = PTR_ERR(cam_mipiswt_sel_h);
		pr_debug("%s : pinctrl err, cam_mipiswt_sel_h\n", __func__);
	}


	cam_mipiswt_sel_l = pinctrl_lookup_state(camctrl, "mipiswt_sel_0");
	if (IS_ERR(cam_mipiswt_sel_l)) {
		ret = PTR_ERR(cam_mipiswt_sel_l);
		pr_debug("%s : pinctrl err, cam_mipiswt_sel_l\n", __func__);
	}
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case CAMMIPI_SWT_EN:
		if (Val == 0)
			pinctrl_select_state(camctrl, cam_mipiswt_en_l);
		else
			pinctrl_select_state(camctrl, cam_mipiswt_en_h);
		break;
	case CAMMIPI_SWT_SEL:
		if (Val == 0)
			pinctrl_select_state(camctrl, cam_mipiswt_sel_l);
		else
			pinctrl_select_state(camctrl, cam_mipiswt_sel_h);
		break;
	default:
...
#define VOL_1000 1000000
	...
	if (On) {
		...
		if (pinSetIdx == 0)
			ISP_MCLK1_EN(1);
		else if ((pinSetIdx == 1) || (pinSetIdx == 2))
			ISP_MCLK2_EN(1);
#endif
		...
		if ((currSensorName && (0 == strcmp(currSensorName, "ov13850mipiraw")))) {
...
#endif
		} else if (currSensorName && (0 == strcmp(currSensorName, "imx258mipiraw"))) {
		...
		else if ((currSensorName && (0 == strcmp(currSensorName, "mn34153mipiraw")))) {
			...
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN,
						pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
#endif
		} else if (currSensorName && (0 == strcmp("ov8858mipiraw", currSensorName))) {
			/* First Power Pin low and Reset Pin Low */
#if 0
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN,
			...
			mtkcam_gpio_set(pinSetIdx, CAMMIPI_SWT_EN, GPIO_OUT_ZERO);
			mtkcam_gpio_set(pinSetIdx, CAMMIPI_SWT_SEL, GPIO_OUT_ZERO);

			mdelay(2);

			/* VCAM_A */
			if (TRUE != _hwPowerOn(VCAMA_PM, VOL_2800)) {
				PK_DBG
				    ("[CAMERA SENSOR] Fail to enable analog power (VCAMA_PM), power id = %d\n",
				     VCAMA_PM);
				goto _kdCISModulePowerOn_exit_;
			}

			/* VCAM_IO */
			if (TRUE != _hwPowerOn(VCAMIO_PM, VOL_1800)) {
				PK_DBG
				    ("[CAMERA SENSOR] Fail to enable IO power (VCAMIO_PM), power id = %d\n",
				     VCAMIO_PM);
				goto _kdCISModulePowerOn_exit_;
			}

			if (TRUE != _hwPowerOn(VCAMD_PM, VOL_1200)) {
				PK_DBG
				    ("[CAMERA SENSOR] Fail to enable digital power (VCAMD_PM),power id = %d\n",
				     VCAMD_PM);
				goto _kdCISModulePowerOn_exit_;
			}
		...
		} else if (currSensorName && (0 == strcmp("hi553mipiraw", currSensorName))) {
			...
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
				mtkcam_gpio_set(pinSetIdx, CAMRST,
						pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
#else
			mtkcam_gpio_set(2, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);

#endif

			mtkcam_gpio_set(pinSetIdx, CAMMIPI_SWT_EN, GPIO_OUT_ZERO);
			mtkcam_gpio_set(pinSetIdx, CAMMIPI_SWT_SEL, GPIO_OUT_ONE);
...
#else
			mtkcam_gpio_set(2, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_ON]);
		...
		if ((currSensorName && (0 == strcmp(currSensorName, "ov13850mipiraw")))) {
			...
			/* VCAM_IO */
			if (TRUE != _hwPowerDown(VCAMIO_PM)) {
				PK_DBG
				    ("[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO),power id = %d\n",
				     VCAMIO_PM);
				/* return -EIO; */
