    ...
    , fci_u16 rx_length)
{
    ...
    struct spi_message    message;
#ifdef FC8080_SPI_MULTI_TRANSFER
    fci_u32 dummy;
    struct spi_transfer    x[2];

    spi_message_init(&message);
    memset(&x, 0, sizeof x);
    dummy = (rx_length + tx_length) % 1024;

    x[0].tx_buf = txbuf;
    x[0].rx_buf = rxbuf;
    x[0].len = tx_length + rx_length;
    if ((rx_length + tx_length) > 1024) {
        x[0].len = tx_length + rx_length - dummy;
    }
    spi_message_add_tail(&x[0], &message);

    if ((rx_length + tx_length) > 1024) {
        x[1].tx_buf = rxbuf;
        x[1].rx_buf = &rxbuf[tx_length + rx_length - dummy];
        x[1].len = dummy;
