...
static int iWriteData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   ...
   EEPROMDB("[BRCB032GWZ3_EEPROM] iWriteData\n" );

   if (ui4_offset + ui4_length >= 0x2000)
   {
      EEPROMDB("[BRCB032GWZ3_EEPROM] Write Error!! S-24CS64A not supprt address >= 0x2000!! \n" );
      return -1;
   }
   ...
   EEPROMDB("[BRCB032GWZ3_EEPROM] iWriteData u4CurrentOffset is %d \n",u4CurrentOffset);
   ...
   EEPROMDB("[BRCB032GWZ3_EEPROM] iWriteData done\n" );
#endif
   return 0;
}
...
#endif//COMMON_CAM_CAL_DRV
{
    ...
    else
    {
        ...
        if(NULL == pBuff)
        {
            EEPROMDB("[BRCB032GWZ3_EEPROM] ioctl allocate mem failed\n");
            return -ENOMEM;
        }
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                EEPROMDB("[BRCB032GWZ3_EEPROM] ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
    ...
    if(NULL == pWorkingBuff)
    {
     ...
     EEPROMDB("[BRCB032GWZ3_EEPROM] init Working buffer address 0x%8x  command is 0x%8x\n", (u32)pWorkingBuff, (u32)a_u4Command);
    ...
    switch(a_u4Command)
    {
        case CAM_CALIOC_S_WRITE:
            EEPROMDB("[BRCB032GWZ3_EEPROM] Write CMD \n");
#ifdef EEPROMGETDLT_DEBUG
        ...
        case CAM_CALIOC_G_READ:
            EEPROMDB("[BRCB032GWZ3_EEPROM] Read CMD \n");
#ifdef EEPROMGETDLT_DEBUG
            do_gettimeofday(&ktv1);
#endif
            ...
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x000CB032) && (EEPROM_Address[0]==0x0) )
            {
            ...
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x000CB032) && (ResetCheck == 0xABCDFEDC) )
            {
                *(u32 *)pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
                EEPROMDB("[BRCB032GWZ3_EEPROM] Reset I2C address %x \n", *(u32 *)pWorkingBuff );
            ...
            else
            {
               ...
               while(0 < readTryagain)
               {
                   ...
                   EEPROMDB("[BRCB032GWZ3_EEPROM] error (%d) Read retry (%d) \n", i4RetValue,readTryagain);
                   if(i4RetValue != 0)
                   {
                       readTryagain--;
                   }
            ...
            EEPROMDB("[BRCB032GWZ3_EEPROM] After read Working buffer data  0x%4x \n", *pWorkingBuff);
            ...
            break;
        default :
               EEPROMDB("[BRCB032GWZ3_EEPROM] No CMD \n");
            i4RetValue = -EPERM;
        break;
    }
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        EEPROMDB("[BRCB032GWZ3_EEPROM] to user length %d \n", ptempbuf->u4Length);
        EEPROMDB("[BRCB032GWZ3_EEPROM] to user  Working buffer address 0x%8x \n", (u32)pWorkingBuff);
        if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pWorkingBuff , ptempbuf->u4Length))
        {
            kfree(pBuff);
            kfree(pWorkingBuff);
            EEPROMDB("[BRCB032GWZ3_EEPROM] ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
{
    EEPROMDB("[BRCB032GWZ3_EEPROM] EEPROM_Open\n");
...
inline static int RegisterEEPROMCharDrv(void)
{
    ...
    {
        EEPROMDB("[BRCB032GWZ3_EEPROM] Allocate device no failed\n");
    ...
    {
        EEPROMDB("[BRCB032GWZ3_EEPROM] Register device no failed\n");
    ...
    if(NULL == g_pEEPROM_CharDrv)
    {
        ...
        EEPROMDB("[BRCB032GWZ3_EEPROM] Allocate mem for kobject failed\n");
    ...
    {
        EEPROMDB("[BRCB032GWZ3_EEPROM] Attatch file operation failed\n");
...
static int EEPROM_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {
int i4RetValue = 0;
    EEPROMDB("[BRCB032GWZ3_EEPROM] Attach I2C \n");
//    spin_lock_init(&g_EEPROMLock);

    //get sensor i2c client
    spin_lock(&g_EEPROMLock); //for SMP
    g_pstI2Cclient = client;
    g_pstI2Cclient->addr = BRCB032GWZ3_DEVICE_ID>>1;
    ...
    if(i4RetValue){
        EEPROMDB("[BRCB032GWZ3_EEPROM] register char device failed!\n");
        return i4RetValue;
    }


    EEPROMDB("[BRCB032GWZ3_EEPROM] Attached!! \n");
    return 0;
}
...
static int __init EEPROM_i2C_init(void)
{
    ...
    if(platform_driver_register(&g_stEEPROM_Driver)){
        EEPROMDB("failed to register BRCB032GWZ3_EEPROM driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stEEPROM_Device))
    {
        EEPROMDB("failed to register BRCB032GWZ3_EEPROM driver, 2nd time\n");
        return -ENODEV;
    }
...
void read_moduleID_BRCB032GWZ3(unsigned char * pinputdata)
{
    unsigned int module_vendor_id = 0x700;
    unsigned int offset = 0xCB032;
    unsigned int module_vendor_id_temp = 0;

    module_vendor_id_temp = (module_vendor_id<<20)&0xfff00000;
    module_vendor_id_temp |= offset;
    iReadDataFromBRCB032GWZ3(module_vendor_id_temp, 2, pinputdata);

    return;
}

#if defined( COMMON_CAM_CAL_DRV)
EXPORT_SYMBOL(BRCB032GWZ3_EEPROM_Ioctl);
#else
