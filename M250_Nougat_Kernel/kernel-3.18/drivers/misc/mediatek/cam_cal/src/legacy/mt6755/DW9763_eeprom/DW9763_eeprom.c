...
int iWriteEEPROM_DW9763(u16 a_u2Addr, u32 a_u4Bytes, u8 *puDataInBytes)
{
	int i4RetValue = 0;
	u32 u4Index = 0;
	...
	if (a_u4Bytes + 2 > 8) {
		EEPROMDB
		    ("[DW9763EEPROM] exceed I2c-mt65xx.c 8 bytes limitation (include address 2 Byte)\n");
		return -1;
	}
	...
	if (i4RetValue != (a_u4Bytes + 2)) {
		EEPROMDB("[DW9763EEPROM] I2C write  failed!!\n");
		return -1;
	}
...
int iReadEEPROM_DW9763(u16 a_u2Addr, u32 ui4_length, u8 *a_puBuff)
{
	int i4RetValue = 0;
	...
	if (ui4_length > 8) {
		EEPROMDB("[DW9763EEPROM] exceed I2c-mt65xx.c 8 bytes limitation\n");
		return -1;
	}
...
int iWriteData_DW9763(unsigned int ui4_offset, unsigned int ui4_length, unsigned char *pinputdata)
{
	int i4RetValue = 0;
	...
	EEPROMDB("[DW9763EEPROM] iWriteData\n");

#if 0
	if (ui4_offset + ui4_length >= 0x2000) {
		EEPROMDB("[DW9763EEPROM] Write Error!! S-24CS64A not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	EEPROMDB("[DW9763EEPROM] iWriteData u4CurrentOffset is %d\n", u4CurrentOffset);
...
#endif
	EEPROMDB("[DW9763EEPROM] iWriteData done\n");
...
int iReadData_DW9763(unsigned int ui4_offset, unsigned int ui4_length, unsigned char *pinputdata)
{
	int i4RetValue = 0;
	int i4ResidueDataLength;
	u32 u4IncOffset = 0;
...
/* EEPROMDB("[S24EEPORM] iReadData\n" ); */
	...
	if (ui4_offset + ui4_length >= 0x2000) {
		EEPROMDB("[DW9763EEPROM] Read Error!! S-24CS64A not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	do {
		if (i4ResidueDataLength >= 8) {
			i4RetValue = iReadEEPROM_DW9763((u16) u4CurrentOffset, 8, pBuff);
			if (i4RetValue != 0) {
				EEPROMDB("[EEPROM] I2C iReadData failed!!\n");
				return -1;
			}
			...
			i4RetValue =
			    iReadEEPROM_DW9763((u16) u4CurrentOffset, i4ResidueDataLength, pBuff);
			if (i4RetValue != 0) {
				EEPROMDB("[EEPROM] I2C iReadData failed!!\n");
				return -1;
			}
...
int DW9763_EEPROM_Ioctl(struct file *file, unsigned int a_u4Command, unsigned long a_u4Param)
#else
...
#endif				/* COMMON_CAM_CAL_DRV */
{
	...
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		if (NULL == pBuff) {
			EEPROMDB("[DW9763EEPROM] ioctl allocate mem failed\n");
			return -ENOMEM;
		}
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			...
			if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param, sizeof(stCAM_CAL_INFO_STRUCT))) {
				kfree(pBuff);
				EEPROMDB("[DW9763EEPROM] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	EEPROMDB("[DW9763EEPROM] init Working buffer address 0x%8x  command is 0x%8x\n",
	...
	case CAM_CALIOC_S_WRITE:
		EEPROMDB("[DW9763EEPROM] Write CMD\n");
#ifdef EEPROMGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		/* i4RetValue = iWriteData_DW9763((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff); */
#ifdef EEPROMGETDLT_DEBUG
	...
	case CAM_CALIOC_G_READ:
		EEPROMDB("[DW9763EEPROM] Read CMD\n");
#ifdef EEPROMGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		EEPROMDB("[DW9763EEPROM] offset %x\n", ptempbuf->u4Offset);
		EEPROMDB("[DW9763EEPROM] length %x\n", ptempbuf->u4Length);
		EEPROMDB("[DW9763EEPROM] Before read Working buffer address 0x%8x\n",
			 (u32) pWorkingBuff);
		...
		EEPROMDB("[DW9763EEPROM] After read Working buffer data  0x%4x\n", *pWorkingBuff);
#else
		ResetCheck = 0;
		...
		if (ptempbuf->u4Offset == 0x800)
			*(u16 *) pWorkingBuff = 0x3;
		else if (((ptempbuf->u4Offset & 0x000FFFFF) == 0x000d9763)
			...
			*(u32 *) pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
			...
			EEPROMDB("[DW9763EEPROM] Init I2C address %x\n", *(u32 *) pWorkingBuff);
		} else if (((ptempbuf->u4Offset & 0x000FFFFF) == 0x000d9763)
			   && (ResetCheck == 0xABCDFEDC)) {
			*(u32 *) pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
			EEPROMDB("[DW9763EEPROM] Reset I2C address %x\n", *(u32 *) pWorkingBuff);
		} else {
			readTryagain = 3;
			while (0 < readTryagain) {
				i4RetValue =
				    iReadDataFromDW9763(ptempbuf->u4Offset, ptempbuf->u4Length,
							pWorkingBuff);
				EEPROMDB("[DW9763EEPROM] error (%d) Read retry (%d)\n", i4RetValue,
					 readTryagain);
				if (i4RetValue != 0)
					readTryagain--;
				else
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		EEPROMDB("[DW9763EEPROM] to user length %d\n", ptempbuf->u4Length);
		EEPROMDB("[DW9763EEPROM] to user  Working buffer address 0x%8x\n",
			 (u32) pWorkingBuff);
		if (copy_to_user
		    ((u8 __user *) ptempbuf->pu1Params, (u8 *) pWorkingBuff, ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pWorkingBuff);
			EEPROMDB("[DW9763EEPROM] ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
...
{
	EEPROMDB("[DW9763EEPROM] EEPROM_Open\n");
	spin_lock(&g_EEPROMLock);
	if (g_u4Opened) {
		spin_unlock(&g_EEPROMLock);
		return -EBUSY;
...
static int RegisterEEPROMCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_EEPROMdevno, 0, 1, EEPROM_DRVNAME)) {
		EEPROMDB("[DW9763EEPROM] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_EEPROMdevno, 1, EEPROM_DRVNAME)) {
		EEPROMDB("[DW9763EEPROM] Register device no failed\n");
	...
	if (NULL == g_pEEPROM_CharDrv) {
		...
		EEPROMDB("[DW9763EEPROM] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pEEPROM_CharDrv, g_EEPROMdevno, 1)) {
		EEPROMDB("[DW9763EEPROM] Attatch file operation failed\n");
...
static unsigned short force[] = {
	EEPROM_I2C_GROUP_ID, DW9763_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END
};
#else
/* static unsigned short force[] = {IMG_SENSOR_I2C_GROUP_ID, DW9763_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END}; */
#endif
...
static int EEPROM_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	EEPROMDB("[DW9763EEPROM] Attach I2C\n");
/* spin_lock_init(&g_EEPROMLock); */
	...
	if (i4RetValue) {
		EEPROMDB("[DW9763EEPROM] register char device failed!\n");
		return i4RetValue;
	}


	EEPROMDB("[DW9763EEPROM] Attached!!\n");
	return 0;
}
...
static int __init EEPROM_i2C_init(void)
{
	...
	if (platform_driver_register(&g_stEEPROM_Driver)) {
		EEPROMDB("failed to register DW9763EEPROM driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stEEPROM_Device)) {
		EEPROMDB("failed to register DW9763EEPROM driver, 2nd time\n");
		return -ENODEV;
	}
