...
int bq2589x_dump_register(void)
{
	...
	if(bq25898s_charger_en==0)return 0;
		...
		for (addr = 0x0; addr < 0x14; addr++) {
			...
			if (ret < 0) {	
				battery_log(BAT_LOG_CRTI, "[bq2589x reg@]read fail\n");
				return ret;
			}else{
				reg[addr]=val;
			}
...
static int bq2589x_parse_dt(struct device *dev, struct bq2589x *bq)
{
	...
	//bq->cfg.enable_term = of_property_read_bool(np, "ti,bq2589x,enable-termination"); dtsi에 설정하는 부분 없음. 
...
static int bq2589x_adjust_absolute_vindpm(struct bq2589x *bq)
{
	...
	ret = bq2589x_set_input_volt_limit(bq, vindpm_volt); /* d번[6~0] */
	if (ret < 0) {
...
int bq2589x_set_charge_profile(struct bq2589x *bq)
{
	...
	ret = bq2589x_set_chargevoltage(bq, bq->cfg.charge_voltage); /* dtsi에는 4400으로 설정했으나 4400으로 설정안됨 check요, 6번[BIT 2~7]  */
	if (ret < 0) {
		dev_err(bq->dev, "%s:Failed to set charge voltage:%d\n", __func__, ret);
		return ret;
	}

	ret = bq2589x_set_chargecurrent(bq, bq->cfg.charge_current); /* dtsi에서 0으로 설정함. 1500으로 변경할 것, 4번[Bit 5 ~ 0]  */
	if (ret < 0) {
		dev_err(bq->dev, "%s:Failed to set charge current:%d\n", __func__, ret);
		return ret;
	}

	ret = bq2589x_set_term_current(bq, bq->cfg.term_current); /* dtsi에는 512로 설정. 5번[3~0] check요  */
	if (ret < 0) {
		dev_err(bq->dev, "%s:Failed to set termination current:%d\n", __func__, ret);
		return ret;
	}

	ret = bq2589x_set_input_current_limit(bq, bq->cfg.iindpm_threshold); /* dtsi에는 1500로 설정, 0번[5~0]*/
	if (ret < 0) {
		dev_err(bq->dev, "%s:Failed to set input current limit:%d\n", __func__, ret);
		return ret;
	}
...
void bq2589x_adapter_in_handler(void)
{
	...
	bq->rsoc = bq2589x_read_batt_rsoc(bq);	/* soc가 95%이상 이면 slave charger를 동작시키지 않는다. 나중에 tuning할 것  */
	if (bq->rsoc > 95 ) {
