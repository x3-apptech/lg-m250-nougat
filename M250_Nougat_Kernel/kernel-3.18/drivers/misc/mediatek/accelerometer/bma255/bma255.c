...
#define SENSOR_DBG(fmt, args...)    pr_debug(SENSOR_TAG"[D]""%s : "fmt, __func__, ##args)
#else
#define SENSOR_FUN(f)               pr_debug(SENSOR_TAG"[F]""%s\n", __func__)
#define SENSOR_ERR(fmt, args...)    pr_err(SENSOR_TAG"[E]""%s %d : "fmt, __func__, __LINE__, ##args)
#define SENSOR_LOG(fmt, args...)    pr_debug(SENSOR_TAG"[L]""%s : "fmt, __func__, ##args)
...
static int bma255_runCalibration(void)
{
	struct i2c_client *client = bma255_i2c_client;
	struct bma255_i2c_data *bma255 = i2c_get_clientdata(client);
	unsigned char backup_offset_x = 0;
	unsigned char backup_offset_y = 0;
	unsigned char backup_offset_z = 0;
	int res = 0;
	int res2 = 0;
	...
	if (atomic_read(&obj->firlen)) {
		...
		for (idx = 0; idx < len; idx++) {
			SENSOR_LOG("[%5d %5d %5d]\n", obj->fir.raw[idx][BMA255_AXIS_X],
				   obj->fir.raw[idx][BMA255_AXIS_Y],
				   obj->fir.raw[idx][BMA255_AXIS_Z]);
		}
...
static ssize_t bma255_softreset_store(struct device_driver *ddri,
...
static ssize_t bma255_runCalibration_store(struct device_driver *ddri,
					   const char *buf, size_t count)
{
	int res = 0;
