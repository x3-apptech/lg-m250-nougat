...
static s16 cali_old[LSM6DS3_ACC_AXES_NUM+1];
/*----------------------------------------------------------------------------*/
typedef enum {
    ADX_TRC_FILTER  = 0x01,
...
/*----------------------------------------------------------------------------*/
static int LSM6DS3_acc_ReadCalibration(int *cal_read)
{
   int fd;
   int i;
   int res;
   ...
   for(i=0;i<6;i++)
   {
	   ...
	   GSE_LOG("lsm6ds3_calibration_read : cal_read[%d]=%d\n",i,cal_read[i]);
   }
   sys_close(fd);
   set_fs(old_fs);
   GSE_LOG("lsm6ds3_calibration_read Done.\n");
...
static int LSM6DS3_acc_WriteCalibration(int *cal)
{
   int fd;
   int i;
   int res;
   ...
   GSE_LOG("lsm6ds3_calibration_save Done.\n");
...
static int LSM6DS3_acc_SetPowerMode(struct i2c_client *client, bool enable)
{
    ...
    /*reset full scale*/
        databuf[0] &= ~LSM6DS3_ACC_RANGE_MASK;
...
static int LSM6DS3_ReadAccData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if(atomic_read(&obj->suspend)){
		GSE_ERR("lsm6ds3 suspend status!!");
		return -1;
	}
	...
	{
	#if 1
		obj->data[LSM6DS3_AXIS_X] = (long)(obj->data[LSM6DS3_AXIS_X]) * obj->sensitivity  /1000* GRAVITY_EARTH_1000/1000; //NTC
...
static int lsm6ds3_do_xl_selftest(const struct lsm6ds3_i2c_data *priv)
{
	int status = 0;
	u8 testset_regs[10] =
	    { 0x30, 0x00, 0x44, 0x00, 0x00, 0x00, 0x00, 0x00, 0x38, 0x00 };
	u8 backup_regs[10];
	...
	// check WHO_AM_I
	status = hwmsen_read_block(client, LSM6DS3_WHO_AM_I, readBuffer, 1);
	if ((status == 0) && (readBuffer[0] == LSM6DS3_FIXED_DEVID)) {
		init_status = 1;
	} else {
		return LSM6DS3_ERR_IDENTIFICATION;
	}
	...
	// self-test setting #1
	status = hwmsen_write_block(client, LSM6DS3_CTRL1_XL, testset_regs, 5);
	if (status != 0) {
		goto XL_HW_SELF_EXIT;
	}
	status = hwmsen_write_block(client, LSM6DS3_CTRL6_C, &testset_regs[5], 5);
	if (status != 0) {
		goto XL_HW_SELF_EXIT;
	}
	...
	do {
		...
		if (!retry)
			break;
	} while (!(readBuffer[0] & LSM6DS3_STATUS_REG_XLDA));

	status =
	    hwmsen_read_block(client, LSM6DS3_OUTX_L_XL, (u8 *) nOutData, 6);
	if (status != 0) {
		goto XL_HW_SELF_EXIT;
	}
	...
	for (i = 0; i < 6; i++) {
		...
		status =
		    hwmsen_read_block(client, LSM6DS3_OUTX_L_XL,
				      (u8 *) nOutData, 6);
		if (status != 0) {
			goto XL_HW_SELF_EXIT;
		}
	...
	// self-test setting #2
	status =
	    lsm6ds3_update_reg(client, LSM6DS3_CTRL5_C, LSM6DS3_ST_XL_MASK,
			       LSM6DS3_ST_XL_ON_POS);
	if (status != 0) {
		goto XL_HW_SELF_EXIT;
	}
	...
	status = lsm6ds3_update_reg(client, LSM6DS3_CTRL1_XL, 0xff, 0x00);
	if (status != 0) {
		goto XL_HW_SELF_EXIT;
	}
	...
	for (i = 0; i < 3; i++) {
		...
		if ((LSM6DS3_ACC_MIN_ST_2G > ST[i])
		    || (LSM6DS3_ACC_MAX_ST_2G < ST[i])) {
			retry++;
		}
	}
...
static ssize_t store_selftest_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
		st_result = lsm6ds3_do_xl_selftest(obj);
...
static int lsm6ds3_do_xl_calibration(struct lsm6ds3_i2c_data *priv)
{
	int res, i = 0;
	int cali[6];
	int sum[3] = { 0 };
	long cnvt[3];
	s16 data[3] = { 0 }, data_prev[3] = {0};
	struct i2c_client *client;
	s16 cali_backup[3];
	...
	res = LSM6DS3_ReadAccRawData(client, data_prev);
	if (res < 0) {
		GSE_ERR("I2C error: ret value=%d", res);
		return LSM6DS3_ERR_SETUP_FAILURE;
	}
	...
	for (i = 0; i < LSM6DS3_CALI_DATA_NUM; i++) {
		mdelay(20);
		res = LSM6DS3_ReadAccRawData(client, data);
		if (res < 0) {
			GSE_ERR("I2C error: ret value=%d", res);
			return LSM6DS3_ERR_SETUP_FAILURE;
		}
	...
	if ((abs(sum[0]) / LSM6DS3_CALI_DATA_NUM > LSM6DS3_TESTLIMIT_XY)
	    || (abs(sum[1]) / LSM6DS3_CALI_DATA_NUM > LSM6DS3_TESTLIMIT_XY)
	    || (abs(sum[2]) / LSM6DS3_CALI_DATA_NUM >
		LSM6DS3_TESTLIMIT_Z_USL_LSB)
	    || (abs(sum[2]) / LSM6DS3_CALI_DATA_NUM <
		LSM6DS3_TESTLIMIT_Z_LSL_LSB)) {
		GSE_ERR("I2C error: ret value=%d", res);
		return LSM6DS3_ERR_SETUP_FAILURE;
	}
  ...
  if(sum[2] < 0) {
	  cnvt[2] = -1 * (sum[2] / LSM6DS3_CALI_DATA_NUM + 8192) ;
  } else {
	  cnvt[2] = -1 * (sum[2] / LSM6DS3_CALI_DATA_NUM - 8192) ;
  }
...
#ifdef CALIBRATION_TO_FILE
	LSM6DS3_acc_WriteCalibration(cali);
#endif
				 ...
				 size_t count)
{
	...
	res = lsm6ds3_do_xl_calibration(data);
	if (LSM6DS3_SUCCESS == res)
		atomic_set(&data->fast_calib_rslt, LSM6DS3_SUCCESS);
	else if (LSM6DS3_ERR_SETUP_FAILURE == res) {
		atomic_set(&data->fast_calib_rslt, 2);
		GSE_ERR("calibration LSM6DS3_ERR_SETUP_FAILURE!\n");
	} else {
	...
	switch(cmd)
	{
		case GSENSOR_IOCTL_SET_ENABLE:
			...
			else
			{
				...
				if(enable == 1)
				{
					err = LSM6DS3_acc_SetPowerMode( obj->client, 1);
				}
				else if(enable == 0)
				{
					err = LSM6DS3_acc_SetPowerMode( obj->client, 0);
				}
			}
			break;

		case GSENSOR_IOCTL_INIT:			
			LSM6DS3_init_client(client, 0);
			break;
		...
		case GSENSOR_IOCTL_SET_CALI:
			...
			{
				err = LSM6DS3_acc_ReadCalibration(cali);
				if(err != 0 ){
...
static int lsm6ds3_setup_eint(void)
{
	...
	obj_i2c_data->irq_node = of_find_compatible_node(NULL, NULL, "mediatek,gyroscope");
	...
	if (obj_i2c_data->irq_node) {
		...
		ret = request_irq(obj_i2c_data->irq, lsm6ds3_eint_handler, IRQF_TRIGGER_FALLING, "gyro-eint", NULL);
		if(ret > 0) {
...
{
	//GSE_FUN();
	const char *name = "mediatek,lsm6ds3_accel";
