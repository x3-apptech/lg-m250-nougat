...
static int bmi160_acc_ReadCalibration_file(struct i2c_client *client)
{
	...
	sscanf(temp_str, "%d %d %d %d %d %d", &cal_read[0],&cal_read[1],&cal_read[2],
			&cal_read[3],&cal_read[4],&cal_read[5]);
	...
	GSE_LOG("data : NEW %d %d %d / OLD %d %d %d\n",
		obj->cali_sw[BMI160_ACC_AXIS_X], obj->cali_sw[BMI160_ACC_AXIS_Y], obj->cali_sw[BMI160_ACC_AXIS_Z],
		obj->cali_sw[BMI160_ACC_AXIS_OLD_X], obj->cali_sw[BMI160_ACC_AXIS_OLD_Y], obj->cali_sw[BMI160_ACC_AXIS_OLD_Z]);
...
static int bmi160_acc_WriteCalibration_file(struct i2c_client *client)
{
	...
	memset(temp_str,0x00, sizeof(temp_str));
	sprintf(temp_str, "%d %d %d %d %d %d",obj->cali_sw[0],obj->cali_sw[1],obj->cali_sw[2],
			obj->cali_sw[3], obj->cali_sw[4],obj->cali_sw[5]);
...
static int make_cal_data_file(void)
{
	...
	memset(temp_str, 0x00, sizeof(temp_str));
	sprintf(temp_str, "%d %d %d %d %d %d", cal_misc[0], cal_misc[1], cal_misc[2],
...
static ssize_t store_cal_update(struct device_driver *ddri, char *buf, size_t count){
...
static int bmi160_acc_enable_nodata(int en)
{
...
#ifdef STEP_DETECT
	if((!atomic_read(&obj->step_detect_enable)) && power == false)
#endif
	...
	atomic_set(&obj->accel_enable, en);
...
static void bmi160_set_interrupt(void)
{
	...
	err = bma_i2c_write_block(obj_i2c_data->client, BMI160_USER_INTR_OUT_CTRL_ADDR, &int_out, 1);
	err += bma_i2c_write_block(obj_i2c_data->client, BMI160_USER_INTR_LATCH_ADDR,&int1_latch,1);
	err += bma_i2c_write_block(obj_i2c_data->client, BMI160_USER_INTR_MAP_0_ADDR, &ex_int, 1);
...
static void bmi160_set_step_mode(int mode)
{
	...
	err += bma_i2c_write_block(obj_i2c_data->client, BMI160_USER_INT_EN_2_ADDR, &step_enable, 1);
	err += bma_i2c_write_block(obj_i2c_data->client, BMI160_USER_STEP_CONFIG_0_ADDR, &config_0, 1);
	err += bma_i2c_write_block(obj_i2c_data->client, BMI160_USER_STEP_CONFIG_1_ADDR, &config_1, 1);

	if(err)
		GSE_ERR("set step mode error\n");
}

static void bmi160_set_step_enable(struct i2c_client *client, int enable)
{
	struct bmi160_acc_i2c_data *obj = obj_i2c_data;
	int err = 0;
	...
	if(enable){
		if(!atomic_read(&obj->step_detect_enable))
		{
			...
			if(!atomic_read(&obj->accel_enable)){
