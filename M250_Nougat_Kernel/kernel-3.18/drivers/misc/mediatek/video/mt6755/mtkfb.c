...
void mtkfb_esd_recovery(void)
{
	if (gSetShutdown != 1)
	{
	    if(_is_enable_esd_check())
	    {
	        primary_esd_external_detected();
	    }
	    else
	    {
	        LG_ESD_recovery();
...
#endif
#if defined(CONFIG_TUNING_IMAGE_ENGINE)
	&dev_attr_enable_image_engine.attr,
#endif
#if defined(CONFIG_TUNING_MIPI_CLOCK)
...
{
#if defined(CONFIG_TUNING_IMAGE_ENGINE) || defined(CONFIG_LGE_VSYNC_SKIP) || defined(CONFIG_TUNING_MIPI_CLOCK) || defined(CONFIG_LGE_READER_MODE)
	int rc;
