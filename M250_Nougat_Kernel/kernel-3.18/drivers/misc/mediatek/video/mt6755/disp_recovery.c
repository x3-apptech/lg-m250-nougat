...
#include <linux/mutex.h>
#include "../../../../../../misc/mediatek/chargepump/inc/rt4832.h"
#endif
#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
#include <soc/mediatek/lge/board_lge.h>
#include "../../../../../../misc/mediatek/chargepump/inc/lm3632.h"
#endif
...
static atomic_t esd_external_detected = ATOMIC_INIT(0);  // ESD status from touch, 0:pass, 1:fail

void primary_esd_external_detected(void)
{
	atomic_set(&esd_external_detected, 1);
}
bool _is_enable_esd_check(void)
{
	return (IS_ESD_ENABLE & primary_get_lcm()->params->dsi.esd_check_enable);
}
#endif
	...
	while (1) {
		...
		if (esd_check_enable) {
			...
			do {
				...
				ret = primary_display_esd_check() | atomic_read(&esd_external_detected);
#else
				...
				if (ret == 1) {
#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
                                        DISPERR("[ESD]esd check fail, will do esd recovery. try=%d, esd_external_detected:%d\n", i, atomic_read(&esd_external_detected));
                                        LG_ESD_recovery();
#else
...
int primary_display_esd_recovery(void)
{
	...
	if (primary_get_state() == DISP_SLEPT) {
        ...
        if(lge_get_maker_id() == 1)  // TOVIS panel
        {
...
#ifdef CONFIG_LGD_INCELL_LG4894_HD_LV5
	if(lge_get_maker_id() == 0)  // LGD panel
	{
		if (primary_display_is_decouple_mode())
...
#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
	if(lge_get_mfts_mode() == 1 && get_lpwg_mfts() != 1)
#else
	if(lge_get_mfts_mode() == 1 && mfts_lpwg != 1)
#endif
	{
		DISPDBG("[ESD]lcm suspend for mfts[begin]\n");
		disp_lcm_suspend_for_mfts(primary_get_lcm());
		DISPMSG("[ESD]lcm suspend for mfts[end]\n");
	}
	else
	{
		DISPDBG("[ESD]lcm suspend for touch[begin]\n");
		disp_lcm_suspend_for_touch(primary_get_lcm());
		DISPMSG("[ESD]lcm suspend for touch[end]\n");
	}
#endif

#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
        DISPDBG("[ESD]lcm resume_power[begin]\n");
#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
        if(lge_get_mfts_mode() == 1 && get_lpwg_mfts() != 1)
#else
        if(lge_get_mfts_mode() == 1 && mfts_lpwg != 1)
        ...
        {
		DISPDBG("[ESD]lcm resume_power[begin]\n");
		disp_lcm_resume_for_mfts(primary_get_lcm());
		DISPMSG("[ESD]lcm resume_power[end]\n");
	}
	else
	{
		DISPDBG("[ESD]lcm resume_power[begin]\n");
		disp_lcm_resume_power(primary_get_lcm());
		DISPMSG("[ESD]lcm resume_power[end]\n");
	}
#endif
...
void LG_ESD_recovery(void)
{
	...
	if(lge_get_board_revno() < HW_REV_C)
		bl_level = old_bl_level;
	else
		bl_level = lm3632_old_bl_level;

	printk("%s Start \n",__func__);

#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
	if(lge_get_board_revno() < HW_REV_C)
		rt4832_chargepump_set_backlight_level(0);
	else
		lm3632_chargepump_set_backlight_level(0);
#else
	chargepump_set_backlight_level(0);
#endif
	mutex_lock(&lg4894_esd_lock);
	primary_display_esd_recovery();
	mutex_unlock(&lg4894_esd_lock);
	mdelay(200);
#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
	if(lge_get_board_revno() < HW_REV_C)
		rt4832_chargepump_set_backlight_level(bl_level);
	else
		lm3632_chargepump_set_backlight_level(bl_level);
#else
	chargepump_set_backlight_level(bl_level);
#endif
	printk("%s End \n",__func__);
}
#elif defined(CONFIG_LGD_INCELL_TD4100_HD_SF3)
