...
// just only use for 4KHZ pwm frequency duty cycle
#define PWM_MAX_ON_LEVEL 1017
#define PWM_MIN_ON_LEVEL 6
#define PWM_MAX_BRIGHTNESS_LEVEL 1020
#endif
...
extern void rt4832_check_pwm_enable(void);
#endif
...
static int disp_pwm_config_init(DISP_MODULE_ENUM module, disp_ddp_path_config *pConfig, void *cmdq)
{
	...
	PWM_MSG("%s : clocksource = %s, clockdiv = %d",__func__,(pwm_src!=1)? "28MHZ" : "104MHZ" , pwm_div);
	...
	PWM_MSG("%s : PWM Frequency = %d KHZ, PWM period = %d",__func__,(28/(pwm_div+1)),1023+1);
#endif
	return 0;
}
...
{
#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5) || defined(CONFIG_LGD_INCELL_TD4100_HD_SF3)
	// prevent to fliker brightness
	if(level_1024 < PWM_MIN_ON_LEVEL)
		level_1024 = PWM_MIN_ON_LEVEL;
	else if(level_1024 > PWM_MAX_ON_LEVEL)
		level_1024 = PWM_MAX_BRIGHTNESS_LEVEL;
#endif
	PWM_MSG("disp_pwm_level_remap : level_1024 = %u)", level_1024);
	return level_1024;
}
...
int disp_pwm_set_backlight_cmdq(disp_pwm_id_t id, int level_1024, void *cmdq)
{
	...
	if (old_pwm != level_1024 || force_update) {
		...
		//QPPS_MSG("%s : bl level = %d, g_pwm_max_backlight = %d",__func__,level_1024,g_pwm_max_backlight[index]);

		reg_base = pwm_get_reg_base(id);
		DISP_REG_MASK(cmdq, reg_base + DISP_PWM_CON_1_OFF, level_1024 << 16, 0x1fff << 16);

		read_width = DISP_REG_GET(reg_base + DISP_PWM_CON_1_OFF) >> 16;
		QPPS_MSG("%s : PWM high width = %d, duty = %d %, read_width = %d ",__func__,level_1024,(int)(((level_1024+1)*100)/1024),read_width);
