...
extern void set_rt4832_switching_freq(int sw_freq);
extern int get_rt4832_switching_freq(void);

static ssize_t store_switching_freq(struct device *dev,struct device_attribute *attr, const char *buf,size_t size)
{

	char *pvalue = NULL;
	int pll_clk = 0;

	pll_clk = simple_strtoul(buf, &pvalue, 10);

	LEDS_DRV_DEBUG("[LCD_tuning] mipi_clk = %d \n", pll_clk);

	set_rt4832_switching_freq(pll_clk);

	return size;
}
static ssize_t show_switching_freq(struct device *dev, struct device_attribute *attr, char *buf)
{
	int freq = 0;

	freq = get_rt4832_switching_freq();
...
static DEVICE_ATTR(switching_freq, 0664, show_switching_freq, store_switching_freq);
#endif
