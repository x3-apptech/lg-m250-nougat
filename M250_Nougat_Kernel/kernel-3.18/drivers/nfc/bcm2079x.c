...
void nfc_timer_timeover(unsigned long arg );

void nfc_timer_registertimer(struct timer_list* ptimer, unsigned long timeover )
{
    init_timer( ptimer );
    ptimer->expires  = get_jiffies_64() + timeover;
    ptimer->data     = NULL;
    ptimer->function = nfc_timer_timeover;
    add_timer( ptimer);
}
void nfc_timer_timeover(unsigned long arg )
{
    printk("wake_unlock nfc_timer_timeover\n");
    if (wake_flag == true) {
                  ...
                  size_t count, loff_t *offset)
{
    ...
    **/
    ...
    nfc_timer_registertimer(timer, TIME_STEP);
    return total;
}
