...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	gf_debug(DEBUG_LOG, "%s: === IOCTL cmd = 0x%04x\n", __func__, cmd);
...
static int gf_read_chip_id(struct gf_device *gf_dev)
{
	...
	for (i = 0; i < 5; i++) {
		gf_spi_read_word_ree(gf_dev, GF3208_REG_CHIP_ID_HI, &chip_id_high);
		gf_spi_read_word_ree(gf_dev, GF3208_REG_CHIP_ID_LO, &chip_id_low);
		chip_id = ( (chip_id_high << 16) | (chip_id_low) ) >> 8;
		gf_debug(INFO_LOG, "%s, i = %d, chip_id_high = 0x%04x, chip_id_low = 0x%04x, chip_id is 0x%08x\n",
				__func__, i, chip_id_high, chip_id_low, chip_id);

		if (chip_id == GF3208_CHIP_ID) {
			gf_debug(INFO_LOG, "%s, succeed to read chip id in probe.\n", __func__);
			break;
		}
...
static int gf_check_irq_rst(struct gf_device *gf_dev)
{
	int i, j;
	...
	gf_debug(INFO_LOG, "%s, irq gpio = %d\n", __func__, gf_dev->irq_gpio);
	...
	for (i = 0; i < 3; i++) {
		...
		gf_spi_write_word_ree(gf_dev, GF3208_REG_IRQ_CTRL0, 0xFFFF); // GF_IRQ_CTRL1 set irq high time
		...
		if (gpio_status_prev == 0 && gpio_status == 0) {
			...
			for (j = 0 ; j < 10 ; j++) {
				...
				if (gpio_status == 1) {
					gf_debug(INFO_LOG, "irq_gpio status high ...\n");
					break;
				}
			}
			if (gpio_status_prev == 0 && gpio_status == 1) {
				gf_debug(INFO_LOG, "%s, [pass] finish irq gpio check\n", __func__);
				result = 0;
				break;
			} else {
			...
			gf_debug(INFO_LOG, "%s, irq_gpio status high..... gpio status = %d w/o delay\n", __func__, gpio_status);
			gf_debug(INFO_LOG, "%s, [pass] finish irq_gpio check\n", __func__);
			result = 0;
			break;
		} else {
	...
	gf_spi_write_word_ree(gf_dev, GF3208_REG_IRQ_CLEAR, 0xFFFF); // GF_IRQ_CTRL2 set irq clear
	mdelay(10);
	gpio_status = gpio_get_value(gf_dev->irq_gpio);
...
//	gf_hw_power_enable(1);
...
#if 0//def SUPPORT_REE_SPI
	/* read chip id */
	for (retry_times = 0; retry_times < GF3208_READ_RETRY_TIMES; retry_times++) {
	...
	if (GF3208_READ_RETRY_TIMES == retry_times) {
	...
	if (GF3208_READ_RETRY_TIMES == retry_times) {
		gf_debug(INFO_LOG, "%s, int pin check failed!\n", __func__);
		status = -ENODEV;
		goto err_class;
	}
	...
	status = gf_check_irq_rst(gf_dev);
	if (status < 0) {
...
{
	gf_debug(ERR_LOG, "%s, lge.fingerprint=%s\n", __func__, str);
...
static int __init gf_init(void)
{
	...
	if (!get_finger_support()) {
		gf_debug(ERR_LOG, "%s, The device doesn't support fingerprint !!!\n", __func__);
		return -ENODEV;
	}
#endif // CONFIG_LGE_FINGERPRINT_ONE_BINARY
