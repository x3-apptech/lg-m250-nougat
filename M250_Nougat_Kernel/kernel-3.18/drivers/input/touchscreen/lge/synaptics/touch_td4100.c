...
static int synaptics_irq_status(struct device *dev, u8 dev_status)
{
	...
	/* ESD status from Display, 0:end, 1:in-progress*/
	atomic_set(&d->state.recovery, esd_recovery_state);

	if (dev_status == 0x81) {
		if (atomic_read(&d->state.init) == INIT_PROGRESS
			|| atomic_read(&d->state.recovery) == ESD_RECOVERY_PROGRESS) {
...
ESD_RECOVERY:
	...
	if (atomic_read(&d->state.recovery) == ESD_RECOVERY_PROGRESS) {
