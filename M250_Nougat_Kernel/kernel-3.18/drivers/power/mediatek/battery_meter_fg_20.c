...
struct battery_meter_table_custom_data {
	...
	BATTERY_PROFILE_STRUCT battery_profile_t0[110];
	int battery_profile_t1_size;
	BATTERY_PROFILE_STRUCT battery_profile_t1[110];
	int battery_profile_t2_size;
	BATTERY_PROFILE_STRUCT battery_profile_t2[110];
	int battery_profile_t3_size;
	BATTERY_PROFILE_STRUCT battery_profile_t3[110];
	int battery_profile_temperature_size;
	BATTERY_PROFILE_STRUCT battery_profile_temperature[110];

	int r_profile_t0_size;
	R_PROFILE_STRUCT r_profile_t0[110];
	int r_profile_t1_size;
	R_PROFILE_STRUCT r_profile_t1[110];
	int r_profile_t2_size;
	R_PROFILE_STRUCT r_profile_t2[110];
	int r_profile_t3_size;
	R_PROFILE_STRUCT r_profile_t3[110];
	int r_profile_temperature_size;
	R_PROFILE_STRUCT r_profile_temperature[110];
};
...
{
	battery_id = (const char*)batt_id;
	bm_debug("battery_id : %s\n", battery_id);
...
int __batt_meter_init_cust_data_from_dt(struct platform_device *dev)
{
...
#ifdef CONFIG_LGE_PM
	/* node change to get battery profile about battery id */
	const char *node_name;
	kal_bool multi_temp_table = KAL_FALSE;

	if (of_property_read_string(np, battery_id, &node_name) == 0)
	{
		np = of_find_node_by_name(np, node_name);
		multi_temp_table = KAL_TRUE;
		bm_err("batt_id: %s, node_name: %s\n", battery_id, node_name);
	} else {
	...
	if(multi_temp_table) {
		if (!of_property_read_u32(np, "batt_temperature_table_size", &val)) {
			...
			multi_temp_table = KAL_FALSE;
			bm_err("No batt_temperature_table in batt_id node."
				"Using common batt_temperature_table\n");
		}
...
#ifdef CONFIG_LGE_PM
	/* node change after getting battery profile */
	np = dev->dev.of_node;
#endif
