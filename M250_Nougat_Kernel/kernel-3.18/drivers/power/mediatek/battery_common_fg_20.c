...
/*PE+ plug out updata by JJB*/
//#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
//int update_pep_chg_det = 0;
//#endif
...
/*PE+ plug out updata by JJB*/
//#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
//kal_bool is_ta_connect_before = KAL_FALSE;
//#endif
...
#endif
	if (tmp32 == 0) {
		battery_log(BAT_LOG_CRTI, "[upmu_is_chr_det] Charger Not exist\n");

/*PE+ plug out updata by JJB*/
//#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)

//		is_ta_connect_before = is_ta_connect;

//		is_ta_connect = KAL_FALSE;
//		ta_check_chr_type = KAL_TRUE;
//		ta_cable_out_occur = KAL_TRUE;

//		battery_log(BAT_LOG_FULL, "%s: [PE+] before = %d is_ta_connect =%d \n", __func__, is_ta_connect_before, is_ta_connect);

//		if (is_ta_connect_before != is_ta_connect) {
//			update_pep_chg_det = 1;
//			wake_up_bat();
//		}

//		battery_log(BAT_LOG_FULL, "[PE+] Cable Setting Reset\n");
//#endif
...
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
	if (mt_usb_is_device()) {
		battery_log(BAT_LOG_CRTI, "[upmu_is_chr_det] Charger exist and USB is not host\n");
		return KAL_TRUE;
	}
...
static int set_safety_timer(int enable)
{
	int ret = 0;
	if (enable) {
		if ((get_pseudo_batt_info(PSEUDO_BATT_MODE) == 1) || (BMT_status.usb_current_max_enabled == 1))
			return -EINVAL;
	}

	ret = battery_charging_control(CHARGING_CMD_ENABLE_SAFETY_TIMER, &enable);
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		/* convert unit 1mA -> 10mA */
		BMT_status.input_current_cc = val->intval / 10;
		break;
#endif
	default:
		ret = -EINVAL;
...
#endif
#if defined (CONFIG_LGE_PM) && defined (CONFIG_LGE_USB_FACTORY)
...
#endif
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED_CC:
		if (BMT_status.battery_charging_enabled_cc == KAL_TRUE)
			val->intval = 1;
		else
			val->intval = 0;
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW: // return ibat
		/* convert 0.1mA -> 1uA */
                                ...
                                const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		if (val->intval)
			...
			BMT_status.battery_charging_enabled = KAL_FALSE;
		break;
#endif
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED_CC:
		if (val->intval)
			BMT_status.battery_charging_enabled_cc = KAL_TRUE;
		else
			BMT_status.battery_charging_enabled_cc = KAL_FALSE;
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		/* convert unit 1mA -> 10mA */
		BMT_status.constant_charge_current_cc = val->intval / 10;
		break;
#endif
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SAFETY_TIMER_SUPPORT
	case POWER_SUPPLY_PROP_SAFETY_TIMER_ENABLE:
		rc = set_safety_timer(val->intval);
		if (rc)
			battery_log(BAT_LOG_CRTI, "set safety timer failed\n");
		break;
#endif
	default:
		rc = -EINVAL;
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SAFETY_TIMER_SUPPORT
	ret = set_safety_timer(!BMT_status.usb_current_max_enabled);
	if (ret)
		battery_log(BAT_LOG_CRTI, " set safety timer failed\n");
#endif
	return size;
...
static void battery_update(struct battery_data *bat_data)
{
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	static unsigned int pre_input_current_max = 0;
	static unsigned int pre_constant_charge_current_max = 0;
#endif
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	{
		...
		/* update status */
		val.intval = bat_data->BAT_STATUS;
		if (lge_battery_get_property(POWER_SUPPLY_PROP_STATUS, &val) == 0)
			bat_data->BAT_STATUS = val.intval;

		/* update health */
		if (lge_battery_get_property(POWER_SUPPLY_PROP_HEALTH, &val) == 0)
			bat_data->BAT_HEALTH = val.intval;
	}
#endif
...
/*PE+ plug out updata by JJB*/
//#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
//	if (update_pep_chg_det) {
//		battery_log(BAT_LOG_CRTI, "[PE+] Update Charger Detech\n");
//		update_pep_chg_det = 0;
//		power_supply_changed(bat_psy);
//	}
//#endif

#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	/* need to notify charger controller when battery information changed */
	if (pre_uisoc2 != BMT_status.UI_SOC2) {
		notify_charger_controller(bat_psy, REQUEST_BY_POWER_SUPPLY_CHANGED);
	} else if (cable_in_uevent) {
		notify_charger_controller(bat_psy, REQUEST_BY_POWER_SUPPLY_CHANGED);
	...
	/* need to notify charger controller when input current changed */
	if (pre_constant_charge_current_max != BMT_status.constant_charge_current_max ||
			pre_input_current_max != BMT_status.input_current_max) {
		if (BMT_status.charger_exist && ac_main.AC_ONLINE)
			notify_charger_controller(&ac_main.psy, REQUEST_BY_POWER_SUPPLY_CHANGED);
		if (BMT_status.charger_exist && usb_main.USB_ONLINE)
			notify_charger_controller(&usb_main.psy, REQUEST_BY_POWER_SUPPLY_CHANGED);
		if (BMT_status.charger_exist && wireless_main.WIRELESS_ONLINE)
			notify_charger_controller(&wireless_main.psy, REQUEST_BY_POWER_SUPPLY_CHANGED);

		pre_input_current_max = BMT_status.input_current_max;
		pre_constant_charge_current_max = BMT_status.constant_charge_current_max;
	}
#endif
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
		notify_charger_controller(ac_psy, REQUEST_BY_POWER_SUPPLY_CHANGED);
#endif
	}
}
...
#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
	if ((usb_status != usb_data->USB_ONLINE) || (is_ta_connect_before != is_ta_connect)) {
//		update_pep_chg_det = 0;
#else
	if (usb_status != usb_data->USB_ONLINE) {
#endif
		usb_status = usb_data->USB_ONLINE;
		pr_info("[JJB] usb_update\n");
		power_supply_changed(usb_psy);
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
		notify_charger_controller(usb_psy, REQUEST_BY_POWER_SUPPLY_CHANGED);
#endif
	}
}
...
{
	PMU_STATUS status = PMU_STATUS_OK;
#if defined(CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT) && defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
...
#endif
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
		BMT_status.input_current_cc = -EINVAL;
		BMT_status.constant_charge_current_cc = -EINVAL;
#endif
...
{
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
	u32 plug_out_aicr = batt_cust_data.usb_charger_current_unconfigured;
#else // MTK original
	...
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
...
#endif
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
			/* enable charger by default */
			battery_charging_control(CHARGING_CMD_SET_HIZ_SWCHR,
			...
			/* Set AICR to 70mA if it is plugged out */
...
/*
#ifdef CONFIG_LGE_PM_CHG_SAFETY_TIMER
	if (BMT_status.safety_timer_enabled != g_previous_safety_timer) {
		g_previous_safety_timer = BMT_status.safety_timer_enabled;
		battery_charging_control(CHARGING_CMD_ENABLE_SAFETY_TIMER, &BMT_status.safety_timer_enabled);
		battery_log(BAT_LOG_CRTI,"[CHARGER] CHARGING_SAFETY_TIMER : %s\n", (BMT_status.safety_timer_enabled ? "Enable" : "Disable"));
	}
#endif
*/
#endif
...
//	unsigned int charging_enable = KAL_TRUE;
	...
	if(BMT_status.bat_exist && lge_get_board_cable() != LT_CABLE_910K) {
