...
void packagelist_destroy(void *pkgl_id, struct super_block *sb)
{
	...
	if (g_pkgl != NULL)
	{
		...
		if (pkgl_dat && pkgl_dat->sb == sb) {
			if (pkgl_dat->thread_id) {
				// stop thread if it is using the sb
				printk(KERN_INFO "sdcardfs: exit thread%d\n", (int)pkgl_dat->thread_id->pid);
				thread = pkgl_dat->thread_id;
				force_sig_info(SIGINT, SEND_SIG_PRIV, thread);
				kthread_stop(thread);
				pkgl_dat->thread_id = NULL;
			}
			pkgl_dat->sb = NULL;
		}

		// remove pkgl_data on last unmount.
		if (pkgl_dat && g_pkgl->access_no == 0)
		{
			remove_all_hashentrys(pkgl_dat);
