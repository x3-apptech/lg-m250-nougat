...
class ClockDumps(RamParser) :
    ...
    def clocks_walker(self, node):
        ...
        dbg_name_address = clk + self.ramdump.field_offset('struct clk', 'name')
        ...
        count_address = clk + self.ramdump.field_offset('struct clk', 'enable_count')
        ...
        output = "{0:40} {1:<25} {2:20} {3:<2}/ {4:<17} {5:<25} v.v (struct clk *)0x{6:<20x}\n".format(
            dbg_name, devid, conid, count, prepare_count, rate, clk)
    ...
    def print_clk_info(self, clk):
        ...
        output = "{0:40} {1:<2}/ {2:<17} {3:<25} v.v (struct clk *)0x{4:<20x}\n".format(dbg_name, count, prepare_count, rate, clk)
    ...
    def clk_providers_walker(self, node):
        ...
        parent_address = node + self.ramdump.field_offset('struct device_node', 'parent')
        parent = self.ramdump.read_word(parent_address, True)
        parent_name_address = parent + self.ramdump.field_offset('struct device_node', 'name')
        parent_name = self.ramdump.read_cstring(self.ramdump.read_word(parent_name_address, True), 48)

        if parent_name == 'clocks':
            self.print_clk_info(data)

        if parent_name == 'soc':
            table_address = data + self.ramdump.field_offset('struct clk_onecell_data', 'clks')
            size_address = data + self.ramdump.field_offset('struct clk_onecell_data', 'clk_num')
            table = self.ramdump.read_word(table_address, True)
            size = self.ramdump.read_word(size_address, True)

            counter = 0
            while counter < size:
                clock_address = table + self.ramdump.field_offset('struct clk_onecell_data', 'clks')
                clk = self.ramdump.read_word(clock_address, True)
                self.print_clk_info(clk)

                counter = counter + 1
                table = table + self.ramdump.sizeof('long')
