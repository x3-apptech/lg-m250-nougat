...
class RTB(RamParser):
    ...
    def get_timestamp(self, rtb_ptr):
        timestamp = self.ramdump.read_structure_field(rtb_ptr, 'struct mtk_rtb_layout', 'timestamp')
        if timestamp == None:
            return 0
    ...
    def print_readlwritel(self, rtbout, rtb_ptr, logtype):
        data = self.ramdump.read_structure_field(rtb_ptr, 'struct mtk_rtb_layout', 'data')
        try:
        ...
        caller = self.ramdump.read_structure_field(rtb_ptr, 'struct mtk_rtb_layout', 'caller')
    ...
    def parse(self):
        ...
        step_size_offset = self.ramdump.field_offset(
            'struct mtk_rtb_state', 'step_size')
        nentries_offset = self.ramdump.field_offset(
            'struct mtk_rtb_state', 'nentries')
        rtb_entry_offset = self.ramdump.field_offset(
            'struct mtk_rtb_state', 'rtb')
        ...
        for i in range(0, step_size):
            rtb_out = self.ramdump.open_file('mtk_rtb{0}.txt'.format(i))
            ...
            if step_size == 1:
                last = self.ramdump.read_int(
                    self.ramdump.addr_lookup('mtk_rtb_idx'))
            else:
                last = self.ramdump.read_int(self.ramdump.addr_lookup(
                    'mtk_rtb_idx_cpu'), cpu=i )
            last = last & mask
            last_ptr = 0
            next_ptr = 0
            next_entry = 0
            ...
            rtb_logtype_offset = self.ramdump.field_offset(
                'struct mtk_rtb_layout', 'log_type')
            rtb_idx_offset = self.ramdump.field_offset(
                'struct mtk_rtb_layout', 'idx')
            while True:
                ...
                if (stop == 1):
                    break
            print_out_str('Wrote RTB to mtk_rtb{0}.txt'.format(i))
            rtb_out.close()
