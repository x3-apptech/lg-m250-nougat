...
from tzdump_noc import tzdump_noc
...
SHARED_IMEM_TZ_BASE_OFF_3 = 0x0000f720
...
tzbsp_diag_boot_info_s = "".join([
  ...
  "I",      # /* PSCI syscall entry CPU Counter */ uint32 psci_entry_cnt;

  "I",      # /* PSCI syscall exit CPU Counter */ uint32 psci_exit_cnt;

  "I",      # /* PSCI warm boot cnt. */ uint32 psci_warm_entry_cnt;

  "I",      # /* PSCI warm boot cnt */ uint32 psci_warm_exit_cnt;

  "Q",      # /* Last Warmboot Jump Address */ uint64 warm_jmp_addr;
...
tzbsp_diag_s ="".join ([
  ...
  "I",      # Wake up info off uint32 wakeup_info_off;
...
class tzdump(RamParser):
  ...
  def print_cpustatus(self, cpu_num, g_tzbsp_diag_boot_info):
    cpustatus = [ "offline", "online", "invalid" ]
    status = len(cpustatus) - 1 # invalid
    str_warmrst = "warm reset"

    warm_entry = g_tzbsp_diag_boot_info[0]
    warm_cnt = g_tzbsp_diag_boot_info[0] - g_tzbsp_diag_boot_info[1]
    pc_cnt = g_tzbsp_diag_boot_info[2] - g_tzbsp_diag_boot_info[3]

    if warm_cnt == 0 :
      status = (warm_entry - (pc_cnt - 1)) if cpu_num == 0 else (warm_entry - pc_cnt)
    elif warm_cnt == 1 :
      return str_warmrst

    if (status < 0) or (status >= len(cpustatus)) : # unexpected status
      status = len(cpustatus) - 1
  ...
  # the dump file for read as well
    try:
      ...
      if ramdump.tz_start == 0xFE800000 :
        ...
        if self.ramdump.hw_id == 8994 :
            ShMemDumpFile.seek(0x645d8,0)
            TZDiagAddr = (struct.unpack ('<I',ShMemDumpFile.read(4)))[0] & 0x000FFFFF
            LogDumpFile = open("{0}/OCIMEM.BIN".format(file_path), "rb")
        elif self.ramdump.hw_id == 8992 :
            ShMemDumpFile.seek(SHARED_IMEM_TZ_BASE_OFF_3,0)
            TZDiagAddr = (struct.unpack ('<I',ShMemDumpFile.read(4)))[0]
            LogDumpFile = open("{0}/DDRCS0_0.BIN".format(file_path), "rb")
      # msm8916
      elif ramdump.tz_start == 0x08600000 :
        ShMemDumpFile = open("{0}/OCIMEM.BIN".format(file_path), "rb")
        ShMemDumpFile.seek(0x720,0)
        if self.ramdump.hw_id == 8976 :
          LogDumpFile = open("{0}/DDRCS1.BIN".format(file_path), "rb")
          TZDiagAddr = (struct.unpack ('<I',ShMemDumpFile.read(4)))[0]
          TZDiagAddr -= 0x80000000
        else :
    ...
    if (TZDiagAddr & 0xFFF): # memory offset not aligned one
      ...
      self.print_out_str_tz(TzOutPutFile, " SHARED IMEM TZ Not Pointing to TZ Log! ")
      sys.exit(0)
    ...
    while True:
      #try:
        ...
        g_tzbsp_diag = struct.unpack(tzbsp_diag_s,LogDumpFile.read(40))  # reading offset variables from dump
        if (g_tzbsp_diag[0] != 0x747A6461):  #0x747A6461 == tzda
          print " TZBSP Log Not available or corrupted! "
          self.print_out_str_tz(TzOutPutFile, " TZBSP Log Not available or corrupted! ")
          sys.exit(0)
        ...
        self.print_out_str_tz(TzOutPutFile, ("CPU Count -> %d " % g_tzbsp_diag[2]))

        # print the VMID's
        self.print_out_str_tz(TzOutPutFile, "\n-------- VMID Status --------- ")
        self.print_out_str_tz(TzOutPutFile, "VMID : VMID Desc")
        ...
        for i in range (0, TZBSP_DIAG_NUM_OF_VMID) :
          ...
          if g_tzbsp_diag_vmid[0] != 0xff:
            self.print_out_str_tz(TzOutPutFile, ("%4x : %s " % (g_tzbsp_diag_vmid[0], self.printable_string (g_tzbsp_diag_vmid[1:7]))))

        # print the Boot status
        self.print_out_str_tz(TzOutPutFile, "\n---------------------------------- Boot Status -------------------------- ")
        self.print_out_str_tz(TzOutPutFile, "CPU : WarmEntry  : WarmExit   : PCEntry     : PCExit     : Warm JumpAddr      : JumpInstr           : Status")
        ...
        for i in range(0,g_tzbsp_diag[2]) :                 # g_tzbsp_diag[2] == CPU Count
          ...
          self.print_out_str_tz(TzOutPutFile, ("%3x : 0x%08x : 0x%08x : 0x%08x  : 0x%08x : 0x%016x : 0x%016x  : %s" % (i,
                                      g_tzbsp_diag_boot_info[0], g_tzbsp_diag_boot_info[1], g_tzbsp_diag_boot_info[2],
                                      g_tzbsp_diag_boot_info[3], g_tzbsp_diag_boot_info[8], g_tzbsp_diag_boot_info[9],
                                      self.print_cpustatus(i, g_tzbsp_diag_boot_info))))

        # print the Reset Table
        self.print_out_str_tz(TzOutPutFile, "\n-------- Reset Status --------- ")
        self.print_out_str_tz(TzOutPutFile, "CPU : Reset Reason/Type : Reset Count ")
        ...
        for i in range (0, g_tzbsp_diag[2]) :  # g_tzbsp_diag[2] == CPU Count
          ...
          try:
            self.print_out_str_tz(TzOutPutFile, ("%3x : 0x%08x (%s) : 0x%08x" % (i, g_diag_reset_info[0], Error_Fatal_table[g_diag_reset_info[0]].ljust(30), g_diag_reset_info[1])))
          except IndexError:
            self.print_out_str_tz(TzOutPutFile, ("%3x : 0x%08x (%s) : 0x%08x" % (i, g_diag_reset_info[0], "UNKNOWN ERROR".ljust(30), g_diag_reset_info[1])))
        ...
        self.print_out_str_tz(TzOutPutFile, "Index: IntInfo : Avail : IntType: IntDesc         : IntrCnts => CPU0              : CPU1              : CPU2              : CPU3              : CPU4              : CPU5")
        ...
        for i in range (0, num_interrupts) :
          ...
          if 1==g_tzbsp_diag[2]:  # for CPU Count 1 , interupt counter will be 4
            ...
            self.print_out_str_tz(TzOutPutFile, ("%5d: 0x%04x  : 0x%03x : 0x%03x  : %15s : IntrCnts => 0x%-16x"
                                               % (i, g_tzbsp_diag_int[0], g_tzbsp_diag_int[1], g_tzbsp_diag_int[3],
                                               self.printable_string(g_tzbsp_diag_int[4:3+TZBSP_MAX_INT_DESC]),
          ...
          if 2==g_tzbsp_diag[2]:  # for CPU Count 2 , interupt counter will be 4
            ...
            self.print_out_str_tz(TzOutPutFile, ("%5d: 0x%04x  : 0x%03x : 0x%03x  : %15s : IntrCnts => 0x%-16x: 0x%-16x"
                                               % (i, g_tzbsp_diag_int[0], g_tzbsp_diag_int[1], g_tzbsp_diag_int[3],
                                               self.printable_string(g_tzbsp_diag_int[4:3+TZBSP_MAX_INT_DESC]),
          ...
          if 4==g_tzbsp_diag[2]:  # for CPU Count 4, interupt counter will be 4
            ...
            self.print_out_str_tz(TzOutPutFile, ("%5d: 0x%04x  : 0x%03x : 0x%03x  : %15s : IntrCnts => 0x%-16x: 0x%-16x: 0x%-16x: 0x%-16x"
                                               % (i, g_tzbsp_diag_int[0], g_tzbsp_diag_int[1], g_tzbsp_diag_int[3],
                                               self.printable_string(g_tzbsp_diag_int[4:3+TZBSP_MAX_INT_DESC]),
                                               g_tzbsp_diag_int_cnt4[0], g_tzbsp_diag_int_cnt4[1],
                                               g_tzbsp_diag_int_cnt4[2],g_tzbsp_diag_int_cnt4[3])))

          if 6==g_tzbsp_diag[2]:  # for CPU Count 6, interupt counter will be 4
            g_tzbsp_diag_int_cnt6 = struct.unpack("QQQQQQ",LogDumpFile.read(48))
            self.print_out_str_tz(TzOutPutFile, ("%5d: 0x%04x  : 0x%03x : 0x%03x  : %15s : IntrCnts => 0x%-16x: 0x%-16x: 0x%-16x: 0x%-16x 0x%-16x: 0x%-16x"
                                               % (i, g_tzbsp_diag_int[0], g_tzbsp_diag_int[1], g_tzbsp_diag_int[3],
                                               self.printable_string(g_tzbsp_diag_int[4:3+TZBSP_MAX_INT_DESC]),
                                               g_tzbsp_diag_int_cnt6[0], g_tzbsp_diag_int_cnt6[1],
                                               g_tzbsp_diag_int_cnt6[2], g_tzbsp_diag_int_cnt6[3],
                                               g_tzbsp_diag_int_cnt6[4], g_tzbsp_diag_int_cnt6[5])))

          if 8==g_tzbsp_diag[2]:  # for CPU Count 4, interupt counter will be 4
            g_tzbsp_diag_int_cnt4 = struct.unpack("QQQQQQQQ",LogDumpFile.read(64))
            self.print_out_str_tz(TzOutPutFile, ("%5d: 0x%04x  : 0x%03x : 0x%03x  : %15s : IntrCnts => 0x%-16x: 0x%-16x: 0x%-16x: 0x%-16x 0x%-16x: 0x%-16x: 0x%-16x: 0x%-16x"
                                               % (i, g_tzbsp_diag_int[0], g_tzbsp_diag_int[1], g_tzbsp_diag_int[3],
                                               self.printable_string(g_tzbsp_diag_int[4:3+TZBSP_MAX_INT_DESC]),
                                               g_tzbsp_diag_int_cnt4[0], g_tzbsp_diag_int_cnt4[1],
                                               g_tzbsp_diag_int_cnt4[2], g_tzbsp_diag_int_cnt4[3],
                                               g_tzbsp_diag_int_cnt4[4], g_tzbsp_diag_int_cnt4[5],
                                               g_tzbsp_diag_int_cnt4[6], g_tzbsp_diag_int_cnt4[7])))

        # Ring Buffer Contents
        self.print_out_str_tz(TzOutPutFile, "\n------------------- ******************************* ----------------- ")
        self.print_out_str_tz(TzOutPutFile, "------ TZBSP DIAG RING BUFF, each line for an individual log ---- ")
        self.print_out_str_tz(TzOutPutFile, "------------------- ******************************* ----------------- ")
        LogDumpFile.close()
        ...
        while line :
          error_code = self.map_to_error(line)
          if error_code:
            line = line + " =====>  TZBSP/QSEE Error!!! => " + error_code
          if "Error Fatal" not in line:
            self.print_out_str_tz(TzOutPutFile, line)
          line = g_tzbsp_ringbuff.readline()
  ...
  def parse(self) :
    ...
    tzdump_noc(format(self.ramdump.outdir) + "/tzdump.txt");
