...
# LGE_CHANGE_S: to create a script according to system 
import platform
local_t32_location = '~/local_bin/t32'
# LGE_CHANGE_E

# LGE_CHANGE_S: get sd card dump info
from dumpinfo import parse_dump_info
# LGE_CHANGE_E

# LGE_CHANGE_S: to check if the ramdump and vmlinux version are matched exactly
...
# LGE_CHANGE_S: ebi pass is used at tzdump.py
        self.ebi_path = options.autodump
# LGE_CHANGE_E
    ...
    def auto_parse(self, file_path):
        ...
        if not self.get_hw_id(add_offset=True):
            return False
...
# LGE_CHANGE_S: parse dump_info.txt
        self.dump_info = parse_dump_info(file_path)
# LGE_CHANGE_E
...
# LGE_CHANGE_S: parse dump_info.txt
                if self.dump_info is not None :
                    extra_start = int(self.dump_info[f], 16)
# LGE_CHANGE_E
    ...
    def create_t32_launcher(self):
        ...
        if t32_host_system != 'Linux':
            ...
            launch_config.write('TMP=${HOME}/tmp\n')
            launch_config.write('SYS=${HOME}/opt/t32\n')
            launch_config.write('HELP=${HOME}/opt/t32/pdf\n')
        launch_config.write('\n')
...
# LGE_ALICE-E_S
        startup_script.write('sys.CONFIG.CoreNumber 8.\n'.encode('ascii', 'ignore'))
        startup_script.write('core.number 8.\n'.encode('ascii', 'ignore'))
        startup_script.write('&max_core=8.\n'.encode('ascii', 'ignore'))
        startup_script.write('&corenumber=0.\n'.encode('ascii', 'ignore'))
        startup_script.write('LOCAL &PATH\n'.encode('ascii', 'ignore'))
        startup_script.write('LOCAL &corenumber_str\n'.encode('ascii', 'ignore'))
# LGE_ALICE-E_E
        startup_script.write('sys.up\n'.encode('ascii', 'ignore'))

# LGE_ALICE-E_S
        startup_script.write('r.s CPSR 0x013\n'.encode('ascii', 'ignore'))
# LGE_ALICE-E_E
...
# LGE_ALICE-E_S
        startup_script.write('while &corenumber<&max_core\n'.encode('ascii', 'ignore'))
        startup_script.write('(\n\tprint &corenumber\n'.encode('ascii', 'ignore'))
        startup_script.write('core.select &corenumber\n'.encode('ascii', 'ignore'))
        startup_script.write('r.s CPSR 0x013\n'.encode('ascii', 'ignore'))
        startup_script.write('&corenumber_str=format.decimal(1.,&corenumber)\n'.encode('ascii', 'ignore'))
        startup_script.write('&PATH="core"+"&corenumber_str"+"_regs.cmm"\n'.encode('ascii', 'ignore'))
        startup_script.write('if (file.exist(&PATH))\n'.encode('ascii', 'ignore'))
        startup_script.write('(\n\tdo &PATH\n'.encode('ascii', 'ignore'))
        startup_script.write(')\n\t&corenumber=&corenumber+1.\n'.encode('ascii', 'ignore'))
        startup_script.write(')\ncore.select 0.\n'.encode('ascii', 'ignore'))
        startup_script.write('core.showactive\n'.encode('ascii', 'ignore'))
        startup_script.write('enddo\n'.encode('ascii', 'ignore'))
# LGE_ALICE-E_E
...
# LGE_ALICE-E_S : rpm t32 script
        rpm_t32_script = open(out_path + '/rpm_t32_startup_script.cmm', 'wb')
        rpm_t32_script.write(('title \"' + out_path + '\"\n').encode('ascii', 'ignore'))
        rpm_t32_script.write('sys.cpu cortexm3\n'.encode('ascii', 'ignore'))
        rpm_t32_script.write('sys.u\n'.encode('ascii', 'ignore'))
        rpm_t32_script.write('area\n'.encode('ascii', 'ignore'))
        for ram in self.ebi_files:
            ebi_path = os.path.abspath(ram[3])
            rpm_t32_script.write('data.load.binary {0} 0x{1:x}\n'.format(
                ebi_path, ram[1]).encode('ascii', 'ignore'))

        rpm_t32_script.write('data.load.binary {0} 0x{1:x}\n'.format(
            os.path.abspath('MSGRAM.BIN'), 0x00060000))
        rpm_t32_script.write('data.load.binary {0} 0x{1:x}\n'.format(
            os.path.abspath('CODERAM.BIN'), 0x00200000))
        rpm_t32_script.write('data.load.binary {0} 0x{1:x}\n'.format(
            os.path.abspath('DATARAM.BIN'), 0x00290000))
        rpm_t32_script.write('data.load.elf {0} /nocode /noclear\n'.format(
            os.path.abspath('RPM_AAAAANAAR.elf')))
        rpm_t32_script.write('data.load.elf {0} 0x00090--0x1036f /noregister\n'.format(
            os.path.abspath('RPM_AAAAANAAR.elf')))

        rpm_t32_script.close()

        tz_t32_script = open(out_path + '/tz_t32_startup_script.cmm', 'wb')

        tz_t32_script.write('sys.cpu CORTEXA53\n')
        tz_t32_script.write('sys.up\n')
        tz_t32_script.write('data.load.binary {0} 0x{1:x}\n'.format(
            os.path.abspath('OCIMEM.BIN'), 0x8600000))
        tz_t32_script.write('data.load.binary {0} 0x{1:x}\n'.format(
            os.path.abspath('DDRCS0.BIN'), 0x20000000))
        tz_t32_script.write('data.load.binary {0} 0x{1:x}\n'.format(
            os.path.abspath('DDRCS1.BIN'), 0x80000000))
        tz_t32_script.write('data.load.binary {0}  0x86500000++0xE6000 /SKIP 0x865E6000 /noclear\n'.format(
            os.path.abspath('DDRCS1.BIN')))
        tz_t32_script.write('data.load.elf {0} /nocode /noclear\n'.format(
            os.path.abspath('qsee.elf')))
        tz_t32_script.write('data.load.elf {0} /nocode /noclear\n'.format(
            os.path.abspath('mon.elf')))
        tz_t32_script.write('PER.Set SPR:0x30200 %QUAD l1\n')
        tz_t32_script.write('PER.Set SPR:0x36200 %QUAD mon_l1\n')
        tz_t32_script.write('mmu.reset\n')
        tz_t32_script.write('mmu.scan\n')
        tz_t32_script.write('mmu.on\n')
...
# LGE_ALICE-E_S
        if t32_host_system != 'Linux':
            ...
            t32_bat.write(')\n')
            t32_bat.write('start %T32_PATH%\t32MARM64.exe -c ./t32_config.t32, ./t32_startup_script.cmm\n')
        else:
            t32_bat = open(out_path + '/launch_t32.sh', 'wb')
            t32_bat.write('~/opt/t32/bin/pc_linux64/t32marm64-qt -c ./t32_config.t32, ./t32_startup_script.cmm\n')
        t32_bat.close()

        if t32_host_system == 'Linux':
            os.chmod(out_path + '/launch_t32.sh', 0775)
